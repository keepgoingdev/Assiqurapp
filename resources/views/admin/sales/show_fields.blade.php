<!-- Id Field -->
<div class="form-group">
    {!! Form::label('id', 'Id:') !!}
    <p>{!! $sale->id !!}</p>
</div>

<div class="form-group">
    {!! Form::label('seller_id', 'Seller ID:') !!}
    <p>{!! $sale->seller_id !!}</p>
</div>

<!-- Age Field -->
<div class="form-group">
    {!! Form::label('age', 'Age:') !!}
    <p>{!! $sale->age !!}</p>
</div>

<!-- Packagetype Field -->
<div class="form-group">
    {!! Form::label('packageType', 'Packagetype:') !!}
    <p>{!! $sale->packageType !!}</p>
</div>

<!-- Ageprice Field -->
<div class="form-group">
    {!! Form::label('agePrice', 'Ageprice:') !!}
    <p>{!! $sale->agePrice !!}</p>
</div>

<!-- Contractortype Field -->
<div class="form-group">
    {!! Form::label('contractorType', 'Contractortype:') !!}
    <p>{!! $sale->contractorType !!}</p>
</div>

<!-- Contractorfirstname Field -->
<div class="form-group">
    {!! Form::label('contractorFirstName', 'Contractorfirstname:') !!}
    <p>{!! $sale->contractorFirstName !!}</p>
</div>

<!-- Contractorlastname Field -->
<div class="form-group">
    {!! Form::label('contractorLastName', 'Contractorlastname:') !!}
    <p>{!! $sale->contractorLastName !!}</p>
</div>

<!-- Contractoraddress Field -->
<div class="form-group">
    {!! Form::label('contractorAddress', 'Contractoraddress:') !!}
    <p>{!! $sale->contractorAddress !!}</p>
</div>

<!-- Contractortaxcode Field -->
<div class="form-group">
    {!! Form::label('contractorTaxCode', 'Contractortaxcode:') !!}
    <p>{!! $sale->contractorTaxCode !!}</p>
</div>

<!-- Contractorbirthday Field -->
<div class="form-group">
    {!! Form::label('contractorBirthday', 'Contractorbirthday:') !!}
    <p>{!! $sale->contractorBirthday !!}</p>
</div>

<!-- Contractorbirthplace Field -->
<div class="form-group">
    {!! Form::label('contractorBirthPlace', 'Contractorbirthplace:') !!}
    <p>{!! $sale->contractorBirthPlace !!}</p>
</div>

<!-- Contractoremail Field -->
<div class="form-group">
    {!! Form::label('contractorEmail', 'Contractoremail:') !!}
    <p>{!! $sale->contractorEmail !!}</p>
</div>

<!-- Contractortelephone Field -->
<div class="form-group">
    {!! Form::label('contractorTelephone', 'Contractortelephone:') !!}
    <p>{!! $sale->contractorTelephone !!}</p>
</div>

<!-- Insuredtype Field -->
<div class="form-group">
    {!! Form::label('insuredType', 'Insuredtype:') !!}
    <p>{!! $sale->insuredType !!}</p>
</div>

<!-- Insuredfirstname Field -->
<div class="form-group">
    {!! Form::label('insuredFirstName', 'Insuredfirstname:') !!}
    <p>{!! $sale->insuredFirstName !!}</p>
</div>

<!-- Insuredlastname Field -->
<div class="form-group">
    {!! Form::label('insuredLastName', 'Insuredlastname:') !!}
    <p>{!! $sale->insuredLastName !!}</p>
</div>

<!-- Insuredaddress Field -->
<div class="form-group">
    {!! Form::label('insuredAddress', 'Insuredaddress:') !!}
    <p>{!! $sale->insuredAddress !!}</p>
</div>

<!-- Insuredtaxcode Field -->
<div class="form-group">
    {!! Form::label('insuredTaxCode', 'Insuredtaxcode:') !!}
    <p>{!! $sale->insuredTaxCode !!}</p>
</div>

<!-- Insuredbirthday Field -->
<div class="form-group">
    {!! Form::label('insuredBirthday', 'Insuredbirthday:') !!}
    <p>{!! $sale->insuredBirthday !!}</p>
</div>

<!-- Insuredbirthplace Field -->
<div class="form-group">
    {!! Form::label('insuredBirthPlace', 'Insuredbirthplace:') !!}
    <p>{!! $sale->insuredBirthPlace !!}</p>
</div>

<!-- Insuredemail Field -->
<div class="form-group">
    {!! Form::label('insuredEmail', 'Insuredemail:') !!}
    <p>{!! $sale->insuredEmail !!}</p>
</div>

<!-- Insuredtelephone Field -->
<div class="form-group">
    {!! Form::label('insuredTelephone', 'Insuredtelephone:') !!}
    <p>{!! $sale->insuredTelephone !!}</p>
</div>

<!-- Deathbentype Field -->
<div class="form-group">
    {!! Form::label('deathBenType', 'Deathbentype:') !!}
    <p>{!! $sale->deathBenType !!}</p>
</div>

<!-- Deathbenfirstname Field -->
<div class="form-group">
    {!! Form::label('deathBenFirstName', 'Deathbenfirstname:') !!}
    <p>{!! $sale->deathBenFirstName !!}</p>
</div>

<!-- Deathbenlastname Field -->
<div class="form-group">
    {!! Form::label('deathBenLastName', 'Deathbenlastname:') !!}
    <p>{!! $sale->deathBenLastName !!}</p>
</div>

<!-- Deathbenaddress Field -->
<div class="form-group">
    {!! Form::label('deathBenAddress', 'Deathbenaddress:') !!}
    <p>{!! $sale->deathBenAddress !!}</p>
</div>

<!-- Deathbentaxcode Field -->
<div class="form-group">
    {!! Form::label('deathBenTaxCode', 'Deathbentaxcode:') !!}
    <p>{!! $sale->deathBenTaxCode !!}</p>
</div>

<!-- Deathbenbirthday Field -->
<div class="form-group">
    {!! Form::label('deathBenBirthday', 'Deathbenbirthday:') !!}
    <p>{!! $sale->deathBenBirthday !!}</p>
</div>

<!-- Deathbenbirthplace Field -->
<div class="form-group">
    {!! Form::label('deathBenBirthPlace', 'Deathbenbirthplace:') !!}
    <p>{!! $sale->deathBenBirthPlace !!}</p>
</div>

<!-- Deathbenemail Field -->
<div class="form-group">
    {!! Form::label('deathBenEmail', 'Deathbenemail:') !!}
    <p>{!! $sale->deathBenEmail !!}</p>
</div>

<!-- Deathbentelephone Field -->
<div class="form-group">
    {!! Form::label('deathBenTelephone', 'Deathbentelephone:') !!}
    <p>{!! $sale->deathBenTelephone !!}</p>
</div>

<!-- Receiveemail Field -->
<div class="form-group">
    {!! Form::label('receiveEmail', 'Receiveemail:') !!}
    <p>{!! $sale->receiveEmail !!}</p>
</div>

<!-- Created At Field -->
<div class="form-group">
    {!! Form::label('created_at', 'Created At:') !!}
    <p>{!! $sale->created_at !!}</p>
</div>

<!-- Updated At Field -->
<div class="form-group">
    {!! Form::label('updated_at', 'Updated At:') !!}
    <p>{!! $sale->updated_at !!}</p>
</div>

<!-- Deleted At Field -->
<div class="form-group">
    {!! Form::label('deleted_at', 'Deleted At:') !!}
    <p>{!! $sale->deleted_at !!}</p>
</div>

